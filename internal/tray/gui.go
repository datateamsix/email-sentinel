...
// openTerminalWithCommand opens a new terminal window and runs a command
// Used for accounts menu and other tray actions
func openTerminalWithCommand(command string, args ...string) {
	go func() {
		var cmd *exec.Cmd
		...
		case "windows":
			// Windows - use cmd
			script := fullCmd + " && pause"
			cmd = exec.Command("cmd", "/c", "start", "cmd", "/k", script)

		case "darwin":
			// macOS - use AppleScript to open Terminal.app
			script := fullCmd + " && read -p 'Press Enter to close...'"
			cmd = exec.Command("osascript", "-e", `tell application "Terminal" to do script "`+script+`"`)
